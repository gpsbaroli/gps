...
class Setting(models.Model):
    ...
    phone_regex = RegexValidator(regex=r'^\+?1?\S\d', message="Phone number must be entered in the format: '+9999 99999'. Up to 20 digits allowed.")
...
class EmailTemplate(models.Model):
    ...
    #body = FroalaField(default="")
    #body = HTMLField(default="")
    body = tinymce_models.HTMLField(default="")
    template_type = models.CharField(max_length=254, default="")
...
class Company(models.Model):
    ...
    status = models.CharField(max_length=15,choices=Statuscom,default="a")
    address_line2 = models.TextField(default="", blank=True)
    ...
    #telephone_number = models.CharField(validators=[phone_regex],unique=True, max_length=17, blank=False,null=True,default=None,help_text="Telephone number must be entered in the format: '+999999999'. Up to 15 digits allowed.")
    company_website = models.CharField(max_length=300,help_text='https://www.example.com/',default="", blank=True)
    company_phone_number = models.CharField(validators=[phone_regex], unique=True,max_length=17, blank=False,null=True,default=None,help_text="Contact Tel No must be entered in the format: '+999999999'. Up to 15 digits allowed.")
    latitude = models.DecimalField(max_digits=9, decimal_places=6,null=True)
    longitude = models.DecimalField(max_digits=9, decimal_places=6,null=True)

    def save(self):
        location = "%s %s %s %s %s" % (self.address, self.address_line2, self.town, self.county, self.post_code)
        #if not self.latitude or not self.longitude:
        latlng = self.geocode(location)
        latlng = latlng.split(',')
        self.latitude = latlng[0]
...
class CouponDetail(models.Model):
    ...
    device_id = models.CharField(blank=False, null=False,max_length=255)
    created_at = models.DateField(("Created at"), default=datetime.now)
...
class Coupon(models.Model):
    ...
    bid = models.FloatField(("Coupon Bid"),default=0,help_text=("Please enter bid amount"),validators=[MinValueValidator(1)])
...
class CompanySettings(models.Model):
    ...
    loyalty_on = models.BooleanField(('On'), default=False,
            help_text=('Activate Loyalty Points'))
    class Meta:
...
class Allergy(models.Model):
    ...
    date = models.DateField(("Created"),blank=True,auto_now_add=True)
