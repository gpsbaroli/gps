...
from .models import Branch,UserData,Coupon,UsersCoupon,Company,EmailTemplate,Setting,LoyaltyManager,CompanyEmailTemplate,Purchase,UserProfile,CompanyRating
...
class CompanyRatingAdmin(admin.ModelAdmin):
    ...
    list_display = ('company','get_customer','get_rating','get_review')
...
class CompanyAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Contact Details', {
            'fields': ('contact_name', 'phone_number', 'email'),
        }),
    )
...
class CouponAdmin(FilterCouponAdmin):
    ...
    list_display = ('image_tag','coupon_code','discount_type','amount','expiry_date','display_users','all_actions')
    search_fields = ('coupon_code','users__Email')
    list_filter = ('expiry_date','discount_type','users__Email')
    readonly_fields = ('image_tag',)
    #form = CouponAdminForm
    list_display_links = ('image_tag', 'coupon_code','all_actions')
    fieldsets = (
        (None, {
           'fields': ('users','image_tag','image','coupon_code','description','discount_type','amount','expiry_date','minimum_spend','maximum_spend')
        }),
    )
    def get_actions(self, request):
        actions = super().get_actions(request)
        ...
        if db_field.name == 'users':
            if request.user.groups.filter(name=settings.NETWORK_GROUP).exists():
                ...
                if(search_by_date):
                    ...
                    get_purchased_users = Purchase.objects.filter(branch__company_id = company_id, userdata_id__in = total_company_users, created_at__gte = six_months_back_date).values('userdata_id').distinct()
